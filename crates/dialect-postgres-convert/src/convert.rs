use crate::protobuf::{
    AConst, AExpr, BoolExpr, BoolExprType, ColumnRef, FuncCall, Node as PbNode, RangeVar, RawStmt,
    ResTarget, SelectStmt, SortBy, a_const, node::Node as NodeEnum,
};
...
fn func_call_to_node(f: &FuncCall) -> Node {
    ...
    let content = if f.agg_star {
        ...
        Node::List {
            items: f.args.iter().map(pb_node_to_node).collect(),
            separator: None,
        }
    };

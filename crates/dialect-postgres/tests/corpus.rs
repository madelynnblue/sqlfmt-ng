use dialect_postgres::PostgresDialect;
use std::path::Path;
...
/// Reads cached corpus files, formats each statement with the PostgreSQL dialect, and updates
/// `testdata/failing/` and `testdata/success/` accordingly. New failures and regressions
/// cause a panic.
/// Run with `cargo test -p dialect-postgres -- --ignored test_external_corpus`.
#[test]
#[ignore]
...
/// Run with `cargo test -p dialect-postgres -- --ignored test_rewrite_corpus`.
#[test]
#[ignore]
fn test_rewrite_corpus() {
    sqlfmt_corpus_tests::run_rewrite_corpus(&PostgresDialect, &testdata_dir());
}
...
fn test_permanent_corpus() {
    sqlfmt_corpus_tests::run_permanent_corpus(&PostgresDialect, &testdata_dir());
}
